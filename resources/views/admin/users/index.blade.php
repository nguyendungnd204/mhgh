                    ...
                    <tbody>
                        @if (count($users) > 0)
                            @foreach ($users as $user)
                                <tr class="border-t border-gray-200 hover:bg-gray-200 cursor-pointer" onclick="window.location.href='{{ route('admin.users.show', $user->id) }}'">
                                    ...
                                    <td class="px-2 py-2 text-gray-600 {{ $user->is_active ? 'bg-green-100 text-green-800' : 'bg-red-500 text-gray-800' }}">
                                    ...
                                    <td class="px-2 py-2" onclick="event.stopPropagation();">
                                        <form action="{{ route('admin.users.update-status', $user->id) }}" method="POST">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit" class="px-3 py-1 text-white rounded-md text-sm transition-colors text-nowrap {{ $user->is_active ? 'bg-red-500 hover:bg-red-700' : 'bg-green-500 hover:bg-green-700' }}">
