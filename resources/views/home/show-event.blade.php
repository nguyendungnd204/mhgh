    ...
    <div class="glass-effect rounded-2xl p-6 md:p-8 shadow-2xl">
        ...
        <div class="text-center mb-6">
            <p class="text-sm md:text-base text-gray-300">
                <span class="font-semibold text-yellow-400">Từ:</span> {{ $event->start_date ? $event->start_date->format('d/m/Y') : '-' }}
                <span class="mx-2">|</span>
                <span class="font-semibold text-yellow-400">Đến:</span> {{ $event->end_date ? $event->end_date->format('d/m/Y') : '-' }}
            </p>
        </div>

        @if ($event->thumbnail)
            <div class="mb-6">
                <img src="{{ asset('storage/' . $event->thumbnail) }}" alt="{{ $event->title ?? 'Event Thumbnail' }}" class="w-full h-auto rounded-lg shadow-md">
            </div>
        @endif
